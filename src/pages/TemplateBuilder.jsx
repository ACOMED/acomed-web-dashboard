...
export default function TemplateBuilder() {
  ...
  const getHandlePos = useCallback((nodeId, handle) => {
    ...
    return {
      x: (hr.left - cr.left - pan.x + hr.width  / 2) / zoom,
      y: (hr.top  - cr.top  - pan.y + hr.height / 2) / zoom,
    };
  }, [zoom, pan]);
  ...
  const onDrop = (e) => {
    ...
    setNodes(prev => [...prev, {
      ...
      type, color: nt.color,
      label: `New ${nt.label}`,
      x: (e.clientX - cr.left - pan.x) / zoom - 80,
      y: (e.clientY - cr.top - pan.y)  / zoom - 40,
      data: { required: false, regPoints: 0, matPoints: 0, triggerCapa: false, capaSeverity: 'minor', category: 'uncategorized' }
    }]);
  };
  ...
  const addNodeToCenter = (type) => {
    ...
    if (canvasRef.current) {
      const cr = canvasRef.current.getBoundingClientRect();
      centerX = (cr.width / 2 - pan.x) / zoom - 80;
      centerY = (cr.height / 2 - pan.y) / zoom - 40;
    }
  ...
  const onHandleMouseDown = (e, nodeId, handle, isOutput) => {
    ...
    setConnecting({
      ...
      sourceHandle: handle,
      startPos: {
        x: (hr.left - cr.left - pan.x + hr.width  / 2) / zoom, // logical coords
        y: (hr.top  - cr.top  - pan.y + hr.height / 2) / zoom,
      }
    });
  };
  ...
  useEffect(() => {
    const onMove = (e) => {
      ...
      if (connecting && canvasRef.current) {
        ...
        setMousePos({ x: (e.clientX - cr.left - pan.x) / zoom, y: (e.clientY - cr.top - pan.y) / zoom });
      }
    };
    const onUp = (e) => {
  ...
  /* ── delete template ── */
  const triggerDeleteTemplate = (e, id) => {
    e.stopPropagation();
          ...
          {savedTemplates.length > 0 && (
            <>
              ...
              {savedTemplates.map((t, i) => (
                <div key={i} style={{ background: '#f5f8fd', border: '1px solid #d6dce6', borderRadius: '8px', padding: '8px', marginBottom: '6px', fontSize: '11px', cursor: 'pointer', position: 'relative' }}
                  onClick={() => { 
                  ...
                  <div style={{ fontWeight: 700, color: '#121319', paddingRight: '20px' }}>{t.name}</div>
                  <div style={{ color: '#88a8d4', marginTop: '2px' }}>{new Date(t.created_at || Date.now()).toLocaleDateString()}</div>
                  <button 
                    onClick={(e) => triggerDeleteTemplate(e, t.id)}
                    style={{ position: 'absolute', top: '8px', right: '8px', background: 'none', border: 'none', color: '#e05c6e', cursor: 'pointer', padding: '2px' }}
                    title="Delete Template"
                  >
                    <svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" strokeWidth="2" fill="none"><path d="M3 6h18"></path><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg>
                  </button>
                </div>
              ))}
            </>
        ...
        <div className="nb-canvas-wrap" id="nbCanvas">
          ...
          {nodes.length === 0 && (
            <div 
              ...
              <div 
                onClick={(e) => { e.stopPropagation(); setShowNodePicker(!showNodePicker); }}
                style={{ 
                  width: '100px', height: '100px', border: '2px dashed #94a3b8', borderRadius: '16px',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  cursor: 'pointer', background: 'rgba(255,255,255,0.8)', transition: 'all 0.2s',
                  color: '#64748b'
                }}
                onMouseOver={e => { e.currentTarget.style.borderColor = '#4a6fa5'; e.currentTarget.style.color = '#4a6fa5'; e.currentTarget.style.background = '#f8fafc'; }}
                onMouseOut={e => { e.currentTarget.style.borderColor = '#94a3b8'; e.currentTarget.style.color = '#64748b'; e.currentTarget.style.background = 'rgba(255,255,255,0.8)'; }}
              >
                <svg viewBox="0 0 24 24" width="36" height="36" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>
              </div>
              <div style={{ marginTop: '16px', color: '#4a6fa5', fontSize: '15px', fontWeight: 600 }}>Add first step...</div>

              {showNodePicker && (
                <div style={{ 
                  position: 'absolute', top: '100%', left: '50%', transform: 'translateX(-50%)', marginTop: '16px',
                  background: '#fff', borderRadius: '12px', boxShadow: '0 10px 25px rgba(0,0,0,0.1)', border: '1px solid #e2e8f0',
                  padding: '8px', width: '220px', display: 'flex', flexDirection: 'column', gap: '4px'
                }}>
                  {NODE_TYPES.map(({ type, icon, label, color }) => (
                    <div 
                      key={type} 
                      onClick={(e) => { e.stopPropagation(); addNodeToCenter(type); }}
                      style={{ 
                        display: 'flex', alignItems: 'center', gap: '12px', padding: '12px',
                        borderRadius: '8px', cursor: 'pointer', fontSize: '14px', fontWeight: 500, color: '#1e293b',
                        transition: 'background 0.2s'
                      }}
                      onMouseOver={e => e.currentTarget.style.background = '#f8fafc'}
                      onMouseOut={e => e.currentTarget.style.background = 'transparent'}
                    >
                      <div style={{ color, background: 'rgba(18,19,25,0.06)', width: '28px', height: '28px', borderRadius: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '14px', fontWeight: 'bold' }}>
                        {icon}
                      </div> 
                      {label}
                    </div>
          ...
          <div
            ...
            onDrop={onDrop}
            onDragOver={(e) => e.preventDefault()}
            onClick={() => { setSelected(null); setShowNodePicker(false); }}
            onMouseDown={(e) => {
              if (e.target.closest('.nb-node')) return;
              setShowNodePicker(false);
          ...
          <div className="nb-toolbar">
            ...
            <button className="nb-tool-btn" onClick={() => setSaveModal(true)} style={{ background: '#4a6fa5', color: '#fff', borderColor: '#4a6fa5', fontWeight: 700 }}>Save Template</button>
            <button className="nb-tool-btn" onClick={() => { setZoom(1); setPan({x:0, y:0}); }}>Reset</button>
          </div>
        </div>
            ...
            <div style={{ padding: '20px' }}>
              ...
              <div style={{ marginBottom: '24px' }}>
                ...
                <div style={{ marginTop: '16px' }}>
                  <label style={{ display: 'block', fontSize: '11px', color: '#334155', marginBottom: '6px', fontWeight: 600 }}>Maturity Category (Radar Axis)</label>
                  <select 
                    value={selectedNode.data?.category || 'uncategorized'} 
              ...
              <div style={{ marginBottom: '24px' }}>
                ...
                {(selectedNode.data?.triggerCapa) && (
                  <div style={{ marginTop: '16px', padding: '16px', background: '#f8fafc', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                    ...
                    <select 
                      value={selectedNode.data?.capaSeverity || 'minor'} 
        ...
        <div className="modal-overlay open">
          <div className="modal-box" style={{ maxWidth: '400px' }}>
            ...
            <div className="modal-foot">
              ...
              <button className="btn" style={{ background: '#e05c6e', color: '#fff', border: 'none' }} onClick={executeDeleteTemplate}>Delete</button>
            </div>
          </div>
        </div>
      )}
