...
function groupByStatus(capas) {
  ...
  capas.forEach(c => {
    ...
    groups[key].push({
      ...
      avatar: c.assignee_id || Math.ceil(Math.random() * 10),
                ...
                {capas[key].map(card => (
                  ...
                  >
                    ...
                    <div className="meta-line">
                      ...
                      <img src={`https://i.pravatar.cc/150?u=${card.avatar}`} alt="avatar" style={{ width: '20px', height: '20px', borderRadius: '50%' }} />
                    </div>
                  </div>
                ))}
              </div>
              <button className="kanban-add-btn">+ Add CAPA</button>
            </div>
          ))}
        </div>
      )}
