...
const req = async (url, opts = {}) => {
  ...
  const response = await fetch(`${BASE_URL}${url}`, { headers, ...opts });
...
export const api = {
  ...
  deleteAudit:   (id)          => req(`/audits/${id}`, { method: 'DELETE' }),
  ...
  getSystemHealth:      () => req('/health'),

  // ── Tenants ───────────────────────────────────────
  getFacilities:   ()     => req('/tenants/facilities'),
  createFacility:  (data) => req('/tenants/facilities', { method: 'POST', body: JSON.stringify(data) }),
  updateFacility:  (id, data) => req(`/tenants/facilities/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
  deleteFacility:  (id)   => req(`/tenants/facilities/${id}`, { method: 'DELETE' }),
  ...
  deleteUser:    (id)   => req(`/tenants/users/${id}`, { method: 'DELETE' }),
  ...
  generateReport: (filters) => {
    ...
    return req(`/reports?${params}`);
  },

  // ── Guides ────────────────────────────────────────
  getGuides:     ()         => req('/guides'),
  uploadGuide:   (formData) => req('/guides', { method: 'POST', body: formData }),
};
