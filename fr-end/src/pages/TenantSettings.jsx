            ...
            <tbody>
              {facilities.length > 0 ? facilities.map(f => (
                <tr key={f.id}>
                  ...
                  <td><button className="btn btn-secondary" style={{ padding: '4px 8px', fontSize: '10px' }}>Edit</button></td>
                </tr>
              ...
              {users.filter(u => u.role === 'Inspector' || u.role === 'inspector').map(u => (
                <div key={u.id} className="inspector-assign-row">
                  ...
                  <label className="toggle-switch">
                    <input type="checkbox" onChange={() => toggleAssign(drawerFacility.id, u.id)} />
